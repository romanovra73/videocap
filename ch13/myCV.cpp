...
#include <nlohmann/json.hpp>
	...
	return "dbname=ml user=videocap password=kSYm4a# host=localhost port=5432";
    }

    bool myCV::motion_detected(Mat new_frame, Mat prev_frame, double pixel_threshold, double total_threshold) {
	if (prev_frame.empty()) {
	    cout <<"Prev frame empty"<<endl;
	    return false;
	}
	if (new_frame.empty()) {
	    cout <<"New frame empty"<<endl;
	    return false;
	}
	imwrite("new_frame.jpg", new_frame);
	imwrite("prev_frame.jpg", prev_frame);
	...
	cout << "Motion total: "<< total << endl;
	cout << "Motion: "<< total<<endl;
