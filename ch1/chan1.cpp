...
int main() {
    ...
    DVR_channel sub_stream(db_work, "SVRD", 1);
	...
	if (!motion_detected)
    	    motion_detected = myCV::motion_detected(frame, prev_frame, 10, 1000);
	if (motion_detected) {
	    ...
	    sub_stream.release();
	    DVR_channel main_stream(db_work, "SVRD", 1, true);
	    int frame_count=0;
		...
		//if (!motion_detected)
		//    motion_detected = myCV::motion_detected(bframe, prev_bframe, 10, 1000);
		if (motion_detected) {
		    frame_count++;
